 ...
 *  2024-05-22 Annotate the WebSocket class as @Component to enable Java WebSocket API in Spring Boot
 *  2024-05-22 Move the thread creation code from WebSocket() constructor to onOpen() event handler
 *  2024-05-23 Add connected argument to sendUpdate() method
 *  2024-05-31 Add PV parameter to sendUpdate() method
 *
 */
package pvwa.ws;
...
@Component /* Enable Java WebSocket API in Spring Boot */
...
//    private final Thread write_thread;
    ...
    public WebSocket()
    {
        logger.log(Level.INFO, "Entering WebSocket()");
...
//        write_thread.start();
    ...
    {
        logger.log(Level.INFO, "Entering onOpen()");
        PVWebSocketContext.register(this);
    ...
    {
        logger.log(Level.INFO, "Entering onClose()");
        dispose();
     ...
     *  @param connected Is the PV connected?
     *  @param pv The PV instance in the PV pool
     */
    public void sendUpdate(final String name, final VType value, final VType last_value, final boolean last_readonly, final boolean readonly, final boolean connected, final PV pv)
    {
        try
        {
            queueMessage(Vtype2Json.toJson(name, value, last_value, last_readonly, readonly, connected, pv));
        }
        catch (final Exception ex)
        {
